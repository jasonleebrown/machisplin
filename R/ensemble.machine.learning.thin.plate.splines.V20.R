...
machisplin.mltps<-function(int.values, covar.ras, n.cores=1, tps=TRUE){
f <- list()
...
if(n.spln>1){i<-seq(1,n.spln)} else {i<-1}# length = number of climate variables
  myLapply_elevOut<-function(i){
			...
			if(tps==FALSE) {
				#out values
				sum.val<-data.frame(out.names[i],f.mod,rsq.model)
				colnames(sum.val)<-c("layer","best model(s):","r2 ensemble:")
				if(i==1){l$n.layers<-length(out.names)}
				l$summary<-sum.val
				#save TPS if R2 is better, else save only model
				names(pred.elev)<- out.names[i]
