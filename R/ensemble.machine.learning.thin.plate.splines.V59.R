			...
			if(tps==TRUE) {
				...
				if(nRx*nCx>1){
					...
					for (h in 1:(nRx*nCx)){
						...
						if (h==1){trim.fat.in<-pred_TPS_elev@file@name
							trim.fat.alpha<-gsub(".grd",'',trim.fat.in)
							trim.fat.beta<-paste0(trim.fat.alpha,".gri")
							trim.fat.all<-c(trim.fat.alpha,trim.fat.beta)}
						if (h>1){trim.fat.in<-pred_TPS_elev@file@name
							trim.fat.alpha<-gsub(".grd",'',trim.fat.in)
							trim.fat.beta<-paste0(trim.fat.alpha,".gri")
							trim.fat.ab<-c(trim.fat.alpha,trim.fat.beta)
							trim.fat.all<-c(trim.fat.ab,trim.fat.all)}
						gc()
						}
						}
				if(nRx*nCx>1){
				...
				for (j in 1:nRx){
				   for (h in 1:nCx){
						...
						if(is.null(feath.ras.TPS)==TRUE){
							...
							trim.fat.in<-pred_TPS_elev@file@name
							trim.fat.alpha<-gsub(".grd",'',trim.fat.in)
							trim.fat.beta<-paste0(trim.fat.alpha,".gri")
							trim.fat.all<-c(trim.fat.alpha,trim.fat.beta)
							}
						else{feath.ras.TPS<-stack(feath.ras,feath.ras.TPS)
							trim.fat.in<-pred_TPS_elev@file@name
							trim.fat.alpha<-gsub(".grd",'',trim.fat.in)
							trim.fat.beta<-paste0(trim.fat.alpha,".gri")
							trim.fat.ab<-c(trim.fat.alpha,trim.fat.beta)
	...
	if(n.spln>1){iter.clim<-seq(1,n.spln)} else {iter.clim<-1}# length = number of climate variables
		for(i in iter.clim){
			...
			if(smooth.outputs.only==FALSE){
				...
				if(round(OptX$p1,2)>OptX.cut){
						OptX.mfit<-paste0(OptX.mfit,"b")#b
						OptX.mfit.wt<-c(OptX.mfit.wt, round(OptX$p1,2))}
						
				if(round(OptX$p2,2)>OptX.cut){
						OptX.mfit<-paste0(OptX.mfit,"g")#g
						OptX.mfit.wt<-c(OptX.mfit.wt, round(OptX$p2,2))}
	
				if(round(OptX$p3,2)>OptX.cut){
						OptX.mfit<-paste0(OptX.mfit,"n")#n
						OptX.mfit.wt<-c(OptX.mfit.wt, round(OptX$p3,2))}
						
				if(round(OptX$p4,2)>OptX.cut){
						OptX.mfit<-paste0(OptX.mfit,"m")#m
						OptX.mfit.wt<-c(OptX.mfit.wt, round(OptX$p4,2))}

				if(round(OptX$p5,2)>OptX.cut){
						OptX.mfit<-paste0(OptX.mfit,"r")#r
						OptX.mfit.wt<-c(OptX.mfit.wt, round(OptX$p5,2))}
					
				if(round(OptX$p6,2)>OptX.cut){
						OptX.mfit<-paste0(OptX.mfit,"v")#v
						OptX.mfit.wt<-c(OptX.mfit.wt, round(OptX$p6,2))}
			...
			if(tps==TRUE) {
				...
				if(nRx*nCx>1){
					rast.list<-as.list(raster_sTPS)
					print("Creating a mosaic of all thin plate splines tiles")
					rast.list$fun <- mean
					rast.mosaic <- do.call(mosaic,rast.list)
					unlink(trim.fat.all)
					trim.fat.all<-NULL
