...
I recently added a new parameter 'smooth.outputs.only' to the 'machisplin.mltps' function.  If 'smooth.outputs.only=TRUE', this removes Boosted Regressive Trees and Random Forests from the options of modeling algorithms. Both occasionally produce blocky outputs. I always recommend first using 'smooth.outputs.only=FALSE' for the intial analyses. Then if you are unhappy with the visual appearance of the created layers, consider 'smooth.outputs.only=TRUE'. Be aware that, if blocky and you exlude these two alogorithms, the predictive performance might dramatically decline (as prior to exlusion, they produced the best model). 

My biggest concern with blocky interpolated surfances, in addtion to looking bad, is the blockiness itself might be the result of overfitting to noise in the training datasets. In most cases, I think smooth continuous surfaces best characterize most spatial processes. Thus, the smoother results might in fact better represent the truth, despite a slightly lower performance. 

![Alt text](https://raw.githubusercontent.com/jasonleebrown/machisplin/master/machisplinParm.jpg?raw=true "Title")
**BIO13 downscaled in northern Madagascar: example of 'smooth.outputs.only' parameter.**  Note that the final results with thin-plate-splinning (TPS) were actually better using the 'smooth.outputs.only=TRUE' (vs. 'smooth.outputs.only=FALSE').  In this case, the TPS was able to better 'fix' the 'smooth.outputs.only=TRUE' ensemble models.  In contrast, the 'smooth.outputs.only=FALSE' ensemble models (without TPS) had a higher r2.  This was beacuse the TPS cannot properly account for the blocky models. In this particular case (and all where r2 ensemble > r2 ensemble + TPS), the final model produced was the r2 ensemble (w/o TPS).  However in all cases, the raw ensemble of 'smooth.outputs.only=FALSE' will always be lower than, or equal to, the model run with 'smooth.outputs.only=TRUE'.   
