...
**New 'smooth.outputs.only' parameter.** Example of a result of BIO13 downscaled in northern Madagascar.  Note that the final results with thin-plate-splinning (TPS) were actually better using the smooth.outputs.only=TRUE (vs. smooth.outputs.only=FALSE).  In this case, the TPS was able to better 'fix' the 'smooth.outputs.only=TRUE' ensemble models.  In contrast, the 'smooth.outputs.only=FALSE' ensemble models (without TPS) were higher.  This was beacuse the TPS cannot properly account for the 'blocky' models and thereby decrease the amount of variance explaned (see r2 values).  In this particular case (and all where r2 ensemble > r2 ensemble + TPS), the final model produced was the r2 ensemble (w/o TPS).  However in all cases, the raw ensemble of 'smooth.outputs.only=FALSE' will always be lower than (or equal to) to the model run with 'smooth.outputs.only=TRUE'.   
