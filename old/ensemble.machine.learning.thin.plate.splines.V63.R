			...
			if(tps==TRUE) {
				...
				if(nRx*nCx>1){
					...
					for (h in 1:(nRx*nCx)){
						...
						b <- c(new.df[[h]][1], new.df[[h]][2], new.df[[h]][3], new.df[[h]][4])
						d <- c(new.df2[[h]][1], new.df2[[h]][2], new.df2[[h]][3], new.df2[[h]][4])
						#crs(b) <- crs(rast_stack)
						#crs(d) <- crs(rast_stack)
						rb <- crop(rast_stack, b)
						#sub sample residuals
						...
						if(nrow(MyTPSdata)<10){
							...
							plot(pred_TPS_elev)
							Sys.sleep(0)}
						if (h>1){raster_sTPS<-stack(raster_sTPS,pred_TPS_elev); pred_TPS_elev<-NULL}
						else {raster_sTPS<-pred_TPS_elev}
						gc()
						}}
				if(nRx*nCx>1){
