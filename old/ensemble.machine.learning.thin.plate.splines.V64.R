...
ALT = terra::rast("alt.tif")
TWI = terra::rast("TWI.tif")
raster_stack<-c(ALT,SLOPE,TWI)
smooth.outputs.only=FALSE; int.values=Mydata; covar.ras=raster_stack; n.cores=1; tps=TRUE
	...
	if(n.spln>1){iter.clim<-seq(1,n.spln)} else {iter.clim<-1}# length = number of climate variables
		for(i in iter.clim){
			...
			if(tps==TRUE) {
				...
				if(nRx*nCx>1){
						rast.mosaic <- terra::mosaic(terra::sprc(raster_sTPS), fun= "mean")
								}
				if(nRx*nCx==1){
					#fit thin plate spline of residuals
				...
				for (j in 1:nRx){
				   for (h in 1:nCx){
						...
						if(is.null(feath.ras.TPS)==TRUE){feath.ras.TPS<-feath.ras}
						else{r_list <- list(feath.ras, feath.ras.TPS)
				...
				if(nRx*nCx>2){
					rast.mosaic.TPS <- terra::mosaic(terra::sprc(feather.ras.TPS), fun= "mean")
					final.TPS<-terra::merge(rast.mosaic.TPS,rast.mosaic)}
				if(nRx*nCx==2){
...
machisplin.write.geotiff<-function(mltps.in, out.names= NULL, overwrite=TRUE){
	...
	#write rasters
	if(is.null(out.names)==TRUE){terra::writeRaster(stack(rast.O), names(rast.O), bylayer=TRUE, format='GTiff',overwrite=overwrite)}
	if(is.null(out.names)==FALSE){terra::writeRaster(stack(rast.O), names(out.names), bylayer=TRUE, format='GTiff',overwrite=overwrite)}
...
##################################################################################################
##################################################################################################
#library(terra);library(gstat);library(MASS);library(nlme);library(fields);library(dismo);library(randomForest);library(earth);library(kernlab);library(mgcv);library(snowfall);library(snow);library(maptools);library(spdep);library(nnet);library(optimx);library(NeuralNetTools);library(breakDown)
