...
stopped at part 3
b<-c(-77,-76,-7,-6)
ALTb <- terra::crop(ALT, b)
SLOPEb <- terra::crop(SLOPE, b)
TWIb <- terra::crop(TWI, b)
raster_stackb<-c(ALTb,SLOPEb,TWIb)
	...
	if(n.spln>1){iter.clim<-seq(1,n.spln)} else {iter.clim<-1}# length = number of climate variables
		for(i in iter.clim){
			...
			if(tps==TRUE) {
				...
				#calculate Sum of squares and resisdual sum of squares
				rss.final <<- sum((dat_tps[[i]]$resp - f.actual) ^ 2)
				l$residd<- cbind((dat_tps[[i]]$resp - f.actual),dat_tps[[i]]$LONG,dat_tps[[i]]$LAT)
				colnames(l$residd)<-c("residuals","long","lat")
				rsq.final <<- 1 - (rss.final/tss) #r squared
